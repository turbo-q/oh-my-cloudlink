      ...
      <div className="bg-elevated border border-app-strong rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5">
          <div className="flex items-baseline gap-3">
            <h2 className="text-xl font-semibold text-app">{t('hosts.connectViaSsh')}</h2>
            <span className="text-sm text-app-faint">{t('sshConfig.subtitle')}</span>
          </div>
          <button onClick={onClose} className="text-app-muted hover:text-app p-1" aria-label={t('common.close')}>
        ...
        <div className="px-6 pb-3 flex gap-3">
          <input
            ...
            placeholder={t('sshConfig.placeholder')}
          />
        ...
        <div className="px-3 max-h-[45vh] overflow-y-auto">
          {loading ? (
            <p className="px-4 py-8 text-sm text-app-subtle text-center">{t('sshConfig.loading')}</p>
          ) : error ? (
            <p className="px-4 py-8 text-sm text-red-400 text-center">{error}</p>
          ) : filtered.length === 0 ? (
            <p className="px-4 py-8 text-sm text-app-subtle text-center">{t('sshConfig.empty')}</p>
          ) : filtered.map((host) => (
        ...
        <div className="px-6 py-4 mt-2 border-t border-app flex items-center justify-between gap-4">
          <span className="text-xs text-app-faint">{t('sshConfig.authHint')}</span>
          <button type="button" onClick={() => void window.electronAPI.sshConfigOpen().catch((err: Error) => alert(err.message))} className="text-sm text-cyan-500 hover:text-cyan-400">{t('sshConfig.openConfig')}</button>
        </div>
      </div>
    </div>
  )
