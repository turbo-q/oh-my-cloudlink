...
- **终端打字跟手**：SSH 连接关闭 Nagle（`TCP_NODELAY`）。此前每个按键的回显会多等一轮 TCP 确认，手感比系统 `ssh` 钝一截，常见要慢 100–200ms，网络不稳时还会跳到几百毫秒。终端、SFTP、端口转发都已关掉这层延迟，打字跟手程度回到真实网络往返
- **空闲断线**：终端与 SFTP 默认每 15 秒 keepalive，避免会话空闲被 NAT / 防火墙掐断
- **未知主机误信任**：首次连接对话框默认是「取消」，回车不会把主机写入 `known_hosts`
- **取消连接卡住**：关掉正在连接的标签时，握手会结束，不再一直挂着
- **终端里的链接**：用系统浏览器打开，不再在应用窗口里加载外部页面
- **设置页路线图**：去掉已完成、且仍写着 FTP 的过时条目

### 新增

- **主机密钥变更**：密钥与 `known_hosts` 不一致时，可点「用新密钥替换」用本次公钥覆盖旧记录并继续连接。默认仍是「关闭」，不会自动改文件；已吊销的密钥不提供替换
- **SFTP 右键菜单**：远程文件支持右键新建文件夹、下载、重命名、删除；行内图标保留，顶部「新建文件夹」按钮去掉
- **应用内确认框**：删除等危险操作改用统一确认弹窗，不再弹出系统 `confirm`
...
- **可复用主机密码（凭证）**：凭证页增加「主机密码」；主机表单可选用库中密码或手动输入，并可选择写入凭证库
- **端口转发列表**：按最近连接排序，展示最近连接时间；支持按名称 / 主机筛选
- **SFTP 路径补全**：本地 / 远程路径栏支持输入与 Tab / ↓ 目录补全（仅主动编辑时弹出，不抢焦点）
- **会话日志导出**：列表项右键可导出或删除日志

### 修复

- **SFTP 远程操作**：Electron 不支持 `window.prompt` 导致新建文件夹 / 重命名无效；行拖拽吞掉单击导致操作需点两次
- **终端搜索关闭**：WebGL 画布盖住关闭按钮时，点 ✕ 只清高亮不关搜索框
- **加密备份恢复**：本地加密备份预览被误判为「未加密备份」
- **会话日志索引**：manifest 原子写入，并自动重建孤儿 `.log`
- **发布脚本**：`publish-release.sh` 兼容 macOS 自带 Bash 3.2（去掉 `mapfile`）

### 发布产物

| 文件 | 说明 |
|------|------|
| `OhMyCloudLink-0.4.1-arm64.dmg` | macOS 安装镜像（Apple Silicon） |
| `OhMyCloudLink-0.4.1-arm64.zip` | macOS 应用包压缩包 |
| `OhMyCloudLink-0.4.1-x64.exe` | Windows 安装包（NSIS） |
