...
- **打字回显延迟**：交互小包改为立即刷出（主进程不再固定等 ~12ms；渲染进程小包跳过 RAF）；`ssh:data` 先于会话日志落盘，避免日志拖慢终端
...
- **SSH 终端输出热路径优化**：会话日志 manifest 写入节流；主进程合并 `ssh:data`（约 12ms / 32KB）；渲染进程按动画帧合并 xterm 写入，减轻刷屏卡顿
